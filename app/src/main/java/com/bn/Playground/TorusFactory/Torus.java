 ...
 * 圆环
 **/
public class Torus implements Graph
{	
    ...
    public Torus(MySurfaceView mv, float rBig, float rSmall, int nCol , int nRow, int texId)
    {
    	this.texId = texId;
    ...
    public void initVertexData(
		...
		for(int i=0;i<nCol;i++){//按照卷绕成三角形的需要
			for(int j=0;j<nRow;j++){//生成顶点编号列表
				int index=i*(nRow+1)+j;//当前四边形第一顶点编号 切成两个就会有三份
			
				alFaceIndex.add(index+1); // 第一个三角形三个顶点的编号入列表 给出三个点就行
