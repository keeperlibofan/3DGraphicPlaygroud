package com.bn.Playground.SoccerFacotry;//包声明
...
public class UtilTools {
	...
	public ResultData initVertexData(float scale, float aHalf, int n) {
		...
		for (int k = 0; k < vertices20.length; k += 9){ //对正二十面体中每个正三角形循环
			float[] v1 = new float[] { vertices20[k], vertices20[k + 1],
					vertices20[k + 2] };
			...
			AlChemicalBondPoints.add(AlCAtomicPosition.get(count * 6));	 //1号顶点的坐标, xyz 三个值
