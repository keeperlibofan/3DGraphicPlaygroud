...
import com.bn.Playground.MathUtil.BezierUtil;
 ...
 * 泰姬陵塔顶部组建1
 */
public class TowerPart implements Graph {
	int texId;
    ...
    public TowerPart(MySurfaceView mv, float scale, int nCol, int nRow, int texId, ArrayList<BNPosition> positions)
    {
    	this.scale = scale;
    	this.texId = texId;
    	//调用初始化顶点数据的initVertexData方法
    	initVertexData(scale,nCol,nRow, positions);
    ...
    public void initVertexData(float scale, int nCol ,int nRow, //大小，列数，行数（行数要保证可以被1除尽）
								ArrayList<BNPosition> positions // 贝塞尔点的位置
	){
		//成员变量初始化
		...
		//加入数据点
		BezierUtil.al.addAll(positions);

		//通过数据点，获取贝赛尔曲线上的点的列表
		ArrayList<BezierUtil.BNPosition> alCurve=BezierUtil.getBezierData(1.0f/nRow);
		//顶点
		...
		vertices=calVertices(alVertix, alFaceIndex);
		...
		//计算卷绕后纹理坐标
		float[] textures= calTextures(alST, alFaceIndex);
    ...
    {
    	//加载顶点着色器的脚本内容
        mVertexShader=ShaderUtil.loadFromAssetsFile("tex.vert", mv.getResources());
        //加载片元着色器的脚本内容
        mFragmentShader= ShaderUtil.loadFromAssetsFile("tex.frag", mv.getResources());
        //基于顶点着色器与片元着色器创建程序
